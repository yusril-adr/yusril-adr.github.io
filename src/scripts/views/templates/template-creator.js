...
const createFrameworkItemTemplate = ({
  ...
  <a href="${url}" target="_blank" rel="noreferrer" class="text-center text-white hover:text-blue-400">
    <img data-src="${src}" alt="${alt[lang]}" class="lazyload h-16 m-auto">
...
const createOddPortfolioTemplate = ({
  ...
  <a href="${url}" target="_blank" rel="noreferrer" class="contents">
    <picture class="portfolio-image md:inline-block">
      <source media="(max-width: ${CONFIG.PORTFOLIO.IMAGES.SMALL}px)" data-srcset="/images/${image.name}-small.webp">
      <source media="(max-width: ${image.size - 1}px)" data-srcset="/images/${image.name}-medium.webp">
      <source data-srcset="/images/${image.name}.webp">
      <img 
        class="lazyload"
        data-src="/images/${image.name}.png" 
        data-srcset="
        ...
        "
        data-sizes="(max-width: ${CONFIG.PORTFOLIO.IMAGES.SMALL}px) ${CONFIG.PORTFOLIO.IMAGES.SMALL}px,(max-width: ${image.size - 1}px) ${CONFIG.PORTFOLIO.IMAGES.MEDIUM}px, ${image.size}px" alt="${name}"
      />
